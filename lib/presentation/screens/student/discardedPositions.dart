...
class DiscardedPositions extends StatefulWidget {
  const DiscardedPositions({super.key, required this.discardedPositions});
  final List<Position> discardedPositions;

  @override
  State<DiscardedPositions> createState() => _DiscardedPositionsState();
}

class _DiscardedPositionsState extends State<DiscardedPositions> {
  void removeDiscarded(Position position) async {
    ...
    await SG.firestore.removeDiscardedPosition(
      position,
      SG.auth.currentUser!.uid,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: Ws.appBar(Ss.discarded, context, HomepageStudent()),
      body: Padding(
